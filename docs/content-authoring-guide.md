...
Challenge는 Academy C1-C4의 성장 흐름을 프로젝트 상세와 분리해 기록한다.
...
현재 `src/data/projects.json`과 `src/data/posts.json`은 기존 화면이 사용하고 있으므로 Ticket 02에서 삭제하지 않는다.

- 새 Content Collections가 이후 화면의 기준 데이터가 된다.
- Projects와 Posts UI를 컬렉션 기반으로 전환하는 티켓에서 기존 JSON 항목을 검증해 옮긴다.
- 전환이 끝나기 전에는 같은 콘텐츠를 두 곳에서 동시에 수정하지 않는다.
