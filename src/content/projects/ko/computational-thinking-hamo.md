...
process:
  ...
  - 단일 코드 읽기에서 두 구조 비교로 이어지는 단계별 학습 흐름을 구현했습니다.
  - 로컬 코멘트 저장과 근거리 공유의 책임을 분리하고 최종 점검 문서로 기능을 확인했습니다.
research: []
keyDecisions:
...
technicalStructure:
  - AppKit TextKit 기반 읽기 전용 코드 뷰어를 SwiftUI 화면에 연결했습니다.
  - SwiftData가 코멘트와 댓글의 source of truth를 담당합니다.
  - MultipeerConnectivity는 같은 수업 세션의 공유와 재동기화를 담당합니다.
challenges:
  - SwiftUI 화면 안에서 TextKit의 텍스트 범위와 줄 번호를 일관되게 연결해야 했습니다.
  - 여러 기기의 코멘트를 공유하면서도 로컬 저장 데이터의 책임을 유지해야 했습니다.
technologies:
  - SwiftUI
  - AppKit TextKit
  - SwiftData
...
whatIWouldChange:
  ...
  - 근거리 공유가 끊겼다가 복구될 때의 충돌 정책을 더 명시적인 테스트로 고정하겠습니다.
learnings:
