...
team:
  type: team
  description: Apple Developer Academy C3 private 팀 프로젝트이며 팀 전체 결과와 공개 가능한 본인 기여를 구분해 설명합니다.
...
context: Apple Developer Academy C3에서 2026년 5월부터 6월까지 진행한 private 팀 프로젝트입니다. 저장소 코드나 내부 협업 정보 없이 공개 가능한 제품 흐름과 본인 기여만 요약합니다.
user: 같은 공간에서 화면을 계속 바라보지 않고 몸을 움직이며 짧은 다인 게임을 함께 즐기려는 사용자를 상정했습니다.
contributions:
...
challenges:
  ...
  - 기기 방향과 주변 환경에 따라 거리·방향 품질이 달라져 센서 값을 그대로 게임 규칙으로 사용할 수 없었습니다.
  - 자동화 test target과 공개 가능한 사용자 테스트 결과가 없어 구현 흐름과 실제 플레이 품질을 분리해 설명해야 합니다.
technologies:
  - Swift
  - SwiftUI
  ...
  - PHASE
outcomes:
  - 저장소에서 방 생성·참가, 역할 배정, 동기화된 카운트다운, 폭탄 전달, 결과와 다음 라운드 흐름을 확인했습니다.
  - 거리·방향 측정, sensor fallback과 공간 음향을 작동 가능한 게임 흐름에 연결했습니다.
  - 사용자 테스트 결과와 플레이 지표는 공개 근거가 없어 성과로 표기하지 않습니다.
whatIWouldChange:
